...
import { StyleSheet, Text, View, FlatList, ScrollView, Modal, Alert, Image, TextInput, TouchableOpacity } from 'react-native';
import { Dropdown } from 'react-native-element-dropdown';
import Toast from 'react-native-toast-message';
import Icon from 'react-native-vector-icons/FontAwesome';
import { useFocusEffect } from '@react-navigation/native';
import { Get as httpGet, PostformData as PostForm } from '../constants/httpService';
...
const SliderScreen = ({ navigation }) => {
    ...
    const [slider, setSlider] = useState({ "SliderId": 0, "SliderImage": "", "OrderBy": "", "IsActive": true, "CreatedAt": null, "CreatedBy": user.userId, "LastUpdatedBy": null, });
    ...
    const renderIdentitiesCard = ({ item }) => (
        <View style={{
            ...
            <Image
                source={item.sliderImage == null || item.sliderImage == "" ? require('../icons/user.png') : { uri: News_URL + item.sliderImage }}
                style={{ width: "auto", aspectRatio: 16/9, marginBottom: 10 }}
            />
            <View style={{ flexDirection: 'row' }}>
                <Text style={{ fontSize: 16 }}>Order By : </Text>
                <Text style={{ fontSize: 16, fontWeight: 'bold', marginBottom: 8 }}>{item.orderBy}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                <TouchableOpacity style={{ marginRight: 10, }} onPress={() => { GetSliderById(item.sliderId); setShowModal(true); }}>
                    <Icon name="pencil" size={20} color={'#5a67f2'} style={{ marginLeft: 8, textAlignVertical: 'center' }} />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => { DeleteSliderIdConfirm(item.sliderId), setShowDelete(true); }}>
                    ...
                    <Modal transparent visible={showModal}>
                        <View style={{
                            ...
                            <View style={{
                                ...
                                <Text style={{ fontSize: 20, marginBottom: 10, color: Colors.shadow, fontWeight: 'bold' }}>Slider</Text>
                                ...
                                <Image
                                    source={slider.SliderImage == null || slider.SliderImage == "" ? require('../icons/user.png') : { uri: News_URL + slider.SliderImage }}
                                    style={{ width: 100, height: 100, marginBottom: 10 }}
                                />
                                <Text style={{ fontSize: 16, marginBottom: 5, color: Colors.secondary }}>Order By :</Text>
                                <TextInput
                                    style={{
                                        borderWidth: 1,
                                        borderColor: Colors.primary,
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                                    ...
                                    <TouchableOpacity style={{
                                        ...
                                        <Text style={{ fontSize: 16, color: Colors.background }}>{slider.SliderId == 0 ? "Add" : "Save"}</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={{
