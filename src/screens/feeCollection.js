...
const StudentBatchFeesScreen = ({route}) => {
    ...
    const GetStudentBatchFeesList = () => {
        ...
        axios.post(`http://192.168.1.3:5291/api/StudentBatchFees/getStudentBatchFeesByStudentBatchId?Id=${studentBatchId}`, JSON.stringify(filter), {
            headers: {
    ...
    const handleSaveStudentBatchFeesDeposit = async () => {
        try {
            if (studentBatchFeesDeposit.Deposit !== 0) {
                await axios.post('http://192.168.1.3:5291/api/StudentBatchFees/post', JSON.stringify(studentBatchFeesDeposit), {
                    headers: {
    ...
    const handleSaveStudentBatchFeesRefund = async () => {
        try {
            if (studentBatchFeesRefund.Refund !== 0) {
                await axios.post('http://192.168.1.3:5291/api/StudentBatchFees/post', JSON.stringify(studentBatchFeesRefund), {
                    headers: {
                        'Content-Type': 'application/json'
                    }
                })
