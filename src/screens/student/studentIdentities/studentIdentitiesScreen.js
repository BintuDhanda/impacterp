    ...
    if (IsFormValid()) {
      try {
        if (studentIdentities.StudentIdentitiesId === 0) {
          await httpPost('StudentIdentities/post', studentIdentities).then(
            response => {
              if (response.status === 200) {
                ...
                Alert.alert(
                  'Success',
                  'StudentIdentities is Added Successfully',
                );
                setStudentIdentities({
                  StudentIdentitiesId: 0,
                  StudentBatchId: studentBatchid,
          ...
          await httpPost('StudentIdentities/put', studentIdentities)
            .then(response => {
              if (response.status === 200) {
                ...
                Alert.alert(
                  'Success',
                  'Update Student Identities Successfully',
                );
                setStudentIdentities({
                  StudentIdentitiesId: 0,
                  StudentBatchId: studentBatchid,
    ...
    <View
      ...
      <View style={{flexDirection: 'row', justifyContent: 'flex-end'}}>
        <TouchableOpacity
          ...
          onPress={() => {
            GetStudentIdentitiesById(item.studentIdentitiesId);
            setShowModal(true);
          }}>
          <Icon
            name="pencil"
        ...
        <TouchableOpacity
          onPress={() => {
            DeleteStudentIdentitiesIdConfirm(item.studentIdentitiesId),
              setShowDelete(true);
          }}>
          <Icon
            name="trash"
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  <Text style={{fontSize: 16}}>
                    {studentIdentities.StringStatus || 'Select Status'}
                  </Text>
                  {showStatusDropdown && (
                    <View
                      style={{
                ...
                <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                  <TouchableOpacity
                    ...
                    <Text style={{fontSize: 16, color: Colors.background}}>
                      {studentIdentities.StudentIdentitiesId == 0
                        ? 'Add'
                        : 'Save'}
                    </Text>
                  </TouchableOpacity>
