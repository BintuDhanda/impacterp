...
const StudentTokenFormScreen = ({route, navigation}) => {
  ...
  const [showValidAdmissionDropdown, setshowValidAdmissionDropdown] =
    useState(false);
    ...
    httpGet(`StudentToken/getById?Id=${tokenId}`)
      .then(response => {
        ...
        setStudentToken({
          ...
          IsValidForAdmissionNonMapped:
            response.data.isValidForAdmissionNonMapped,
        });
      })
      .catch(err => {
  ...
  const IsFormValid = () => {
    if (studentToken.ValidFrom.length == 0) {
      ShowError('Select Valid From');
      return false;
    }
    if (studentToken.ValidUpto.length == 0) {
      ShowError('Select Valid Upto');
      return false;
    }
    if (studentToken.TokenFee.length == 0) {
      ShowError('Enter Valid Token Fee');
      return false;
    }
    if (studentToken.BatchId.length == 0) {
        ...
        <ScrollView
          ...
          <View
            ...
            <TextInput
              ...
              value={
                studentToken.ValidFrom === '' || studentToken.ValidFrom === null
                  ? 'Select Valid From'
                  : getFormattedDate(studentToken.ValidFrom)
              }
          ...
          <View
            ...
            <TextInput
              style={{marginLeft: 10, fontSize: 16, color: Colors.secondary}}
              value={
                studentToken.ValidUpto === '' || studentToken.ValidUpto === null
                  ? 'Select Valid Upto'
                  : getFormattedDate(studentToken.ValidUpto)
              }
          ...
          <TouchableOpacity
            ...
            <Text style={{fontSize: 16}}>
              {studentToken.IsValidForAdmissionNonMapped || 'Select Valid'}
            </Text>
