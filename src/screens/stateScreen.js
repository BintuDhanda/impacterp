  ...
  const fetchStatesByCountryId = async () => {
    try {
      const response = await httpGet(
        `State/getStateByCountryId?Id=${countryId}`,
      );
    ...
    if (IsFormValid()) {
      try {
        if (state.StateId !== 0) {
          ...
          await httpPost('State/post', state).then(response => {
            if (response.status === 200) {
              fetchStatesByCountryId();
    ...
    <View
      ...
      <View style={{flexDirection: 'row'}}>
        <TouchableOpacity
          ...
          onPress={() => handleEditState(item.stateId)}>
          <Icon
            name="pencil"
            size={20}
            color={'#5a67f2'}
        ...
        <TouchableOpacity
          style={{marginRight: 10}}
          onPress={() => handleNavigate(item.stateId, item.stateName)}>
          <Icon
            name="cogs"
            size={20}
        ...
        <TouchableOpacity
          onPress={() => {
            DeleteStateIdConfirm(item.stateId);
            setShowDelete(true);
          }}>
          <Icon
            name="trash"
